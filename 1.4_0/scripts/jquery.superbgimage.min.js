...
(function($) {
    jQuery.fn.superbgimage = function(loadingoptions) {
        ...
        $('#' + options.id).children('img').each(function() {	//	img 空数组？还没元素
            $(this).attr('rel', $.superbg_imgIndex++);	//	未用到？
        ...
        $(this).children('a').each(function() {	//	这里需要改造，原来已加载a链接列表
            $(this).attr('rel', $.superbg_imgIndex++).click(function() {	//	给链接添加点击逻辑，superbg_imgIndex从1开始
        ...
        //$.superbg_imgIndex--;
		if($.myFileList == undefined)
			$.myFileList = [];
		$.superbg_imgIndex = $.myFileList.length;
    ...
    jQuery.fn.superbgLoad = function() {
        ...
        if (options.randomimage === 1) {
            $.superbg_imgActual = (1 + parseInt(Math.random() * ($.superbg_imgIndex - 1 + 1), 10));		//$.superbg_imgIndex可能为string，-1变为number
        }
        $(this).superbgShowImage($.superbg_imgActual);
    };
    jQuery.fn.superbgimagePreload = function() {	//	不实用，本地延迟主要来自加载到内存，这个动作会阻塞CPU，引起动画卡顿
		//if (console && console.log) console.log("call superbgimagePreload");
		//alert(this);
        var options = $.extend($.fn.superbgimage.defaults, $.fn.superbgimage.options);
		if ($.superbg_preload !== 0) {
			clearInterval($.superbg_preload);
			//	if (console && console.log) console.log("clearInterval superbgimagePreload " + $.superbg_firstLoaded);
		}
        if (!$.superbg_firstLoaded && ($('#' + $.superbg_saveId).children('a').length > 0)) {
		//if (!$.superbg_firstLoaded && ($('#' + $.superbg_saveId).children('a').length > 0)) {
            $.superbg_preload = setInterval("$(this).superbgimagePreload()", 111);
			//	if (console && console.log) console.log("set superbgimagePreload " + $.superbg_firstLoaded);
            return;
        }
        $('#' + $.superbg_saveId).children('a.preload:first').each(function() {
            ...
            $(img).load(function() {
                ...
                if ($('#' + options.id).children('*' + "[rel='" + imgrel + "']").length === 0) {
                    ...
                    $('#' + options.id).prepend(this);	//插入图片，一般走不到这里？
                }
                img.onload = function() {
                };
				if (console && console.log) console.log("img " + imgrel + " " + imgtitle + " " + href + " preloaded.");
            }).error(function() {
                img.onerror = function() {
                };
            }).attr('src', href);
            $.superbg_preload = setInterval("$(this).superbgimagePreload()", 111);
			//	if (console && console.log) console.log("set superbgimagePreload " + imgrel);
		//}).removeClass('preload');	//	原版没写auto，但CSS selector却用a.preload,a.auto导致翻页时找不到下一个a
        }).removeClass('preload').addClass('auto');
    };
    jQuery.fn.startSlideShow = function() {
		...
		if(!($.myFileList[$.superbg_imgActual - 1].isvideo)){
			//图片
			$.superbg_interval = setInterval("$(this).nextSlide()", options.slide_interval);
			//if (console && console.log) console.log("setInterval nextSlide 1 " + options.slide_interval);
		} else {
			//视频 延迟修正
			if(
				...
				var duration = Math.round($('#' + options.id).children('video' + "[rel='" + $.superbg_imgActual + "']")[0].duration*1000);
				$.superbg_interval = setInterval("$(this).nextSlide()", duration);
				//if (console && console.log) console.log("setInterval nextSlide 2 " + duration);
			}
			else
			{
				$.superbg_interval = setInterval("$(this).nextSlide()", options.slide_interval);
				//if (console && console.log) console.log("setInterval nextSlide 3 " + options.slide_interval);
			}
		}
    ...
    jQuery.fn.nextSlide = function() {
        ...
        // if ($.superbg_slideshowActive) {		//	为啥要取消？
            // clearInterval($.superbg_preload);
        // }
        $.superbg_direction = 0;
		...
		var filteredList = $.myFileList.filter(a => !a.moved);
		...
		//$.superbg_imgSlide = filteredList[($.superbg_imgSlide>=newlength) ? 0 : $.superbg_imgSlide].rel;	//	FIXME: 逻辑错误，superbg_imgSlide为序号，可轻易超过过滤后的大小
		if(newlength > 0){
			var next = filteredList.find(a => a.rel > Number($.superbg_imgSlide));
			if(next == undefined)	//	找不到更大的
				$.superbg_imgSlide = filteredList[0].rel;
			else
				$.superbg_imgSlide = next.rel;
		}
		
		// console.log("afa nextSlide $.superbg_imgSlide=" + $.superbg_imgSlide);
        //if ($.superbg_imgSlide > $.superbg_imgIndex) {
            //$.superbg_imgSlide = $('#thumbs1 a.preload,a.auto')[0].rel;	//	$.superbg_imgSlide = 1;	// 可能不是1哦，因为可能被删了
			//$.superbg_imgSlide = filteredList[0].rel;	//	$.myFileList.find(a => !a.moved).rel;
        //}
        // if (options.randomimage === 1) {
		...
		if (options.randomimage === 1) {
			...
			$.superbg_imgSlide = filteredList[parseInt(Math.random() * newlength, 10)].rel;
            while ($.superbg_imgSlide === $.superbg_imgLast) {
                //$.superbg_imgSlide = $('#thumbs1 a.preload,a.auto')[parseInt(Math.random() * newlength, 10)].rel;
				$.superbg_imgSlide = filteredList[parseInt(Math.random() * newlength, 10)].rel;
            }
    ...
    jQuery.fn.prevSlide = function() {
		...
		//$.superbg_imgSlide = filteredList[($.superbg_imgSlide<=1) ? (newlength - 1) : ($.superbg_imgSlide-2)].rel;
		if(newlength > 0){
			var prev = filteredList.reverse().find(a => a.rel < Number($.superbg_imgSlide));
			if(prev == undefined)	//	找不到更大的
				$.superbg_imgSlide = filteredList[0].rel;
			else
				$.superbg_imgSlide = prev.rel;
		}
		
        //if ($.superbg_imgSlide < 1) {
            //	$.superbg_imgSlide = $('#thumbs1 a.preload,a.auto')[newlength - 1].rel;	//	$.superbg_imgSlide = $.superbg_imgIndex;
			//$.superbg_imgSlide = filteredList[newlength - 1].rel;
        //}
        // if (options.randomimage === 1) {
            // $.superbg_imgSlide = (1 + parseInt(Math.random() * ($.superbg_imgIndex - 1 + 1), 10));
            // while ($.superbg_imgSlide === $.superbg_imgLast) {
		...
		//	$.superbg_imgActual = ($('#thumbs1 a.preload,a.auto')[$.superbg_imgSlide - 1].rel);
		//	$.superbg_imgActual = filteredList[$.superbg_imgSlide>=1 ? ($.superbg_imgSlide - 1) : (newlength - 1)].rel;
	...
	jQuery.fn.afa_moveto = function(dest) {
		...
		$.myFileList[$.superbg_imgActual - 1].moved = true;
		var thisfilename = $.myFileList[$.superbg_imgActual - 1].title;
    ...
    jQuery.fn.superbgResize = function() {
        ...
        var options = $.extend($.fn.superbgimage.defaults, $.fn.superbgimage.options);
        var thisimg = $.superbg_thisimg;	//	$('#' + options.id + ' img.activeslide,video.activeslide');	//	这里过于耗时
		if(thisimg == null)
			return;
			//	$(':root > body:first-of-type > #' + options.id + ':first-of-type > img.activeslide,video.activeslide');
			//	:root > body:first-of-type > #superbgimage:first-of-type > img.activeslide,video.activeslide
		//var thisimg1 = $(thisimg);
        var dimensions = $(this).superbgCalcSize(thisimg.width(), thisimg.height());
		//if (console && console.log) console.log("call superbgResize1 " + dimensions);
		if(dimensions == null){
			return;
		}
    ...
    jQuery.fn.superbgCalcSize = function(imgw, imgh) {	//	旧版这里是传递的img经缩放后的当前大小，非原始大小
		//if (console && console.log) console.log("call superbgCalcSize " + imgw + "," + imgh);
        var options = $.extend($.fn.superbgimage.defaults, $.fn.superbgimage.options);
		var window1 = $(window);
		...
		var newleft = isNaN(afa_xA) ? Math.round((browserwidth - newwidth) / 2) : afa_xA;	//	afa_xA afa_xA为上次计算的位置坐标 若未定义，默认为中心点
        var newtop = isNaN(afa_yA) ? Math.round((browserheight - newheight) / 2) : afa_yA;
		var now = new Date().getTime();
		if(afa_imageload){
			afa_oldtime = now;
			if( options.initposition === 1 )	//	左上角
			{
				newleft = 0;
				newtop = 0;
			}
			else if( options.initposition === 2 )	//	右上角
			{
				newleft = Math.round(browserwidth - newwidth);
				newtop = 0;
			}
			else if( options.initposition === 3 )	//	左下角
			{
				newleft = 0;
				newtop = Math.round(browserheight - newheight);
			}
			else if( options.initposition === 4 )	//	右下角
			{
				newleft = Math.round(browserwidth - newwidth);
				newtop = Math.round(browserheight - newheight);
			}
		...
		if(!afa_drag || options.scaletofit === 2){	//在屏幕边界则自动滚动
			...
			if(options.scaletofit != 2 || afa_notmove){		//	非全屏，在边缘滚动
				if(afa_borderX>browserwidth*0.925-10 && newleft+newwidth>browserwidth){		//	数值定义了四周边界范围
				//	在右侧边，而且右面还有部分图片未显示
			...
			if(notmove ){//|| afa_notmove && options.scaletofit == 2){
				...
				if( $.superbg_bordermove == 0 ){	//60fps 影响垂直同步
					...
					//requestAnimationFrame(()=>{$(this).superbgResize();});
				}
			}
		}
		if(afa_zoom!=1.0 || afa_zoom_old!=1.0){
			...
			if( typeof(e) != 'undefined' ){
				...
				var options = $.extend($.fn.superbgimage.defaults, $.fn.superbgimage.options);
				var thisimg = $($.superbg_thisimg);	//$($('#' + options.id + ' img.activeslide,video.activeslide'));
		...
		//if (console && console.log) console.log("call superbgCalcSize1 " + newwidth + "," + newheight + "," + newleft + "," + newtop);
        return rcarr;
    };
    
    jQuery.fn.superbgShowImage = function(img) {
		//if (console && console.log) console.log("call superbgShowImage " + img);
        var options = $.extend($.fn.superbgimage.defaults, $.fn.superbgimage.options);
        $.superbg_imgActual = $(this).attr('rel');
        if (typeof img !== 'undefined') {
            $.superbg_imgActual = Number(img);	//	$(this)不知是啥，实际用的序号来自参数传递；img为字符串，rel属性值
        }
        ...
        if ($('#' + options.id).children('*' + "[rel='" + $.superbg_imgActual + "']").length === 0
			&& $.myFileList.length > 0) {	//从列表中读src、title等配制
			...
			var myFile = $.myFileList[$.superbg_imgActual - 1];
			imgsrc = myFile.href;
			imgtitle = myFile.title;
			isvideo = myFile.isvideo;
			type = myFile.type;
        } else {	//不知道是什么情况
    ...
    jQuery.fn.superbgLoadImage = function(imgsrc, imgtitle, isvideo, type) {
		//if (console && console.log) console.log("call superbgLoadImage " + imgsrc + "," + imgtitle + "," + isvideo + "," + type);
		afa_zoom = 1.0;
		afa_zoom_old = 1.0;
		afa_xA = 0;
		afa_yA = 0;
        ...
        if ($('#' + options.id).children('*' + "[rel='" + $.superbg_imgActual + "']").length === 0) {
			//if(isvideo !== 'true') {
            if((typeof(isvideo) == 'boolean' && !isvideo) || (typeof(isvideo) == 'string' && isvideo !== 'true')) {
				//图片
				var img = new Image();
				$(img).load(function() {
					$(this).css('position', 'absolute').hide();
				...
				$(obj).css('position', 'absolute').hide();
					...
					obj.onloadedmetadata = function() {
						...
						if(options.slide_interval <= obj.duration*1000 - options.outspeed){
							...
							var duration = Math.round(obj.duration*1000 - options.outspeed);
							$.superbg_interval = setInterval("$(this).nextSlide()", duration);
							//if (console && console.log) console.log("setInterval nextSlide 4 " + duration);
						}
					};
			}
        } else {
    ...
    jQuery.fn.superbgTransition = function(thisimg, dimensions) {
		//if (console && console.log) console.log("call superbgTransition " + thisimg + "," + dimensions);
        var options = $.extend($.fn.superbgimage.defaults, $.fn.superbgimage.options);
        var newwidth = dimensions[0];
        var newheight = dimensions[1];
        var newleft = dimensions[2];
        ...
        $(thisimg).css('z-index', 1).css('will-change','left, top, width, height');	//	https://developer.mozilla.org/en-US/docs/Web/CSS/will-change
        ...
        if (options.transition === 0) {
            ...
            }).addClass('activeslide');	//	在这里显示新图片，style生效，之前是隐藏的
        } else if (options.transition === 1) {	//	渐变
			//console.log("fadeIn1 " + options.speed + " " + new Date().getTime());
            $(thisimg).fadeIn(options.speed, function() {
				...
				//console.log("fadeIn2 " + new Date().getTime());
            }).addClass('activeslide');	//	在这里显示新图片，style生效，之前是隐藏的
			//console.log("fadeIn3 " + options.speed + " " + new Date().getTime());
        } else {
		...
		$.superbg_thisimg = $('#' + options.id + ' img.activeslide,video.activeslide');
        if (akt_transitionout === 1) {
            ...
            if (options.transition === 0) {
				...
				//console.log("fadeOut1 " + outspeed + " " + new Date().getTime());
                $('#' + options.id + ' img.lastslide,video.lastslide').fadeOut(outspeed).removeClass('lastslide');
				//console.log("fadeOut2 " + outspeed + " " + new Date().getTime());
            } else {
                if (options.transition === 2) {
                    theEffect = 'slide';
                    theDir = 'down';
